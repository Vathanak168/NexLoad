...
def _kb_main_menu(user_id):
    """Build Main Menu — shows admin options if authorized."""
    markup = InlineKeyboardMarkup(row_width=2)

    if _is_admin(user_id):
        # Admin section
        markup.row(
            InlineKeyboardButton("🔑 Key Management", callback_data="nav:keys"),
        )
        markup.row(
            InlineKeyboardButton("📊 Server Dashboard", callback_data="nav:dashboard"),
            InlineKeyboardButton("📋 All Licenses", callback_data="nav:list_keys")
        )

    # User section (available to everyone)
    markup.row(
        InlineKeyboardButton("🎬 How to Download", callback_data="nav:help_dl"),
...
def _kb_after_keygen():
    """After key is generated."""
    markup = InlineKeyboardMarkup(row_width=2)
    markup.row(
        InlineKeyboardButton("➕ Create Another Key", callback_data="nav:create_key"),
        InlineKeyboardButton("📋 List All Keys", callback_data="nav:list_keys")
    )
    markup.row(InlineKeyboardButton("↩️ Back to Menu", callback_data="nav:main"))
    return markup
...
def _text_main_menu(user_first, is_admin):
    ...
    if is_admin:
        admin_section = (
            "👑 <b>Admin Panel</b>\n"
            "┣ 🔑 <b>Key Management</b> — Create, list, validate, revoke keys\n"
            "┣ 📊 <b>Server Dashboard</b> — Live download stats\n"
            "┗ 📋 <b>All Licenses</b> — Browse all keys with status\n\n"
        )

    return (
        f"👋 <b>NexLoad Control Panel</b>\n"
        f"Welcome, <b>{user_first}</b>!\n\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"{admin_section}"
        "🛠️ <b>Tools</b>\n"
        "┣ 🎬 <b>Download</b> — Paste any video URL to download\n"
        "┣ 🔍 <b>Validate</b> — Check if a license key is valid\n"
        "┗ 🌐 <b>Web App</b> — Open NexLoad in browser\n\n"
        "━━━━━━━━━━━━━━━━━━━━━\n"
        "💡 <i>Paste any video URL directly to download!</i>"
    )
...
def register_key_handlers(bot):
    ...
    def on_confirm_gen(call):
        ...
        try:
            ...
            _safe_edit(bot, call, text, _kb_after_keygen())
        except Exception as e:
...
def _send_stats_msg(bot, message):
    ...
    if os.path.exists(stats_file):
        try:
            ...
            bot.reply_to(
                message,
                f"📊 Downloads: <b>{st.get('total_downloads',0):,}</b> | "
                f"Data: <b>{st.get('total_bytes',0)//(1024*1024):,} MB</b>",
                parse_mode="HTML"
            )
        except Exception as e:
            bot.reply_to(message, f"❌ {e}")
    else:
        bot.reply_to(message, "📊 No stats recorded yet.")
